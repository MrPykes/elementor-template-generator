...
function createSection(columns = [], settings = {}){
  return {
    id: uid('section'),
    elType: 'section',
    settings: settings || {},
...
function createColumn(children = [], settings = {}){
  return {
    id: uid('column'),
    elType: 'column',
    settings: Object.assign({ width: settings.width || '100%' }, settings),
    elements: children
  };
}
...
function textWidgetFromHtml(html){
  return createWidget('text-editor', { editor: html });
}

function headingWidget(text, level){
  return createWidget('heading', { title: text, size: `h${level}` });
}

function imageWidget(src, alt){
  return createWidget('image', { image: { url: src || 'https://via.placeholder.com/600x400', alt: alt||'' } });
}

function buttonWidget(text, href){
  return createWidget('button', { text: text || 'Button', link: { url: href || '#' } });
}
    ...
    if(kids.length === 1 && (kids[0].tagName || '').toLowerCase() === 'div'){
      ...
      if(isColumns){
        const cols = inner.map(ch => {
          ...
          return createColumn(widgets, { width: Math.floor(100 / inner.length) + '%' });
        });
        return [ createSection(cols, { html_tag: 'section' }) ];
      }
    }

    // Fallback: collect widgets and wrap in single column
    const widgets = kids.flatMap(k => convertNode(k, $));
    return [ createSection([ createColumn(widgets) ], { html_tag: 'section' }) ];
  }
  ...
  if(tag === 'div'){
    ...
    if(isColumns){
      const cols = kids.map(ch => {
        const widgets = childrenElements($(ch)).flatMap(c => convertNode(c, $));
        return createColumn(widgets, { width: Math.floor(100 / kids.length) + '%' });
      });
      return [ createSection(cols) ];
    }

    // Otherwise treat as simple grouping -> section with single column
    const widgets = kids.flatMap(k => convertNode(k, $));
    return [ createSection([ createColumn(widgets) ]) ];
  }
...
function convertHtmlToElementor(html, title = 'Converted Template'){
  ...
  return {
    version: '3.0.0',
    exported_by: 'html-to-elementor-json-mvp',
